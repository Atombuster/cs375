...
void myfree(void *p){
	...
	struct block *curr = PTR_OFFSET(p, -PADDED_SIZE(sizeof(struct block)));
	...
	while(temp->next != NULL){
		if(temp->in_use == 0 && temp->next->in_use == 0){
			//printf("%d + %ld = %ld \n", temp->size, temp->next->size+PADDED_SIZE(sizeof(struct block)),temp->size + temp->next->size + PADDED_SIZE(sizeof(struct block)));
			
			temp->size += temp->next->size + PADDED_SIZE(sizeof(struct block));
